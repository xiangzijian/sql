...
-- 统计范围：2025年全年，按城市、月份、紧急标识分
WITH 
-- 1. 获取首次修改服务时间的记录（从操作历史表，仅客户发起的改约）
...
main_order_data AS (
    SELECT 
        ...
        a.urgent_flag,
        CASE 
            WHEN a.urgent_flag IN (1, 2) THEN '紧急单'  -- 紧急单和紧急转非紧急都算紧急单
        ...
        CASE 
            WHEN a.urgent_flag = 0  -- 只有非紧急单按预约时间判断
                ...
                AND a.first_sign_time != '1000-01-01 00:00:00'
                AND (
                    -- 情况1：不管有无致电，首次签到时间小于等于预约服务结束时间，则是准时上门
                    (a.service_end_time IS NOT NULL
                     AND a.first_sign_time <= a.service_end_time)
                    OR
                    -- 情况2：未致电情况下客户改约，首次签到时间小于等于客户首次改约的服务结束时间，就算准时上门
    ...
    WHERE a.pt = '20260114000000'
        ...
        -- 不过滤 first_sign_time，让没有签到时间的订单也计入分母
...
SELECT 
    ...
    -- 紧急单统计（包括 urgent_flag = 1 和 2）
    COUNT(DISTINCT CASE WHEN urgent_flag IN (1, 2) THEN order_no END) AS `紧急单总数`,
    COUNT(DISTINCT CASE WHEN urgent_flag IN (1, 2) AND is_urgent_ontime = 1 THEN order_no END) AS `紧急单2小时上门数`,
    CONCAT(
        ROUND(
            CASE 
                WHEN COUNT(DISTINCT CASE WHEN urgent_flag IN (1, 2) THEN order_no END) = 0 THEN 0
                ELSE COUNT(DISTINCT CASE WHEN urgent_flag IN (1, 2) AND is_urgent_ontime = 1 THEN order_no END) * 100.0 
                     / COUNT(DISTINCT CASE WHEN urgent_flag IN (1, 2) THEN order_no END)
            END, 
        2), '%'
    ) AS `紧急单2小时上门率`,
    
    -- 非紧急单统计（只有 urgent_flag = 0）
    COUNT(DISTINCT CASE WHEN urgent_flag = 0 THEN order_no END) AS `非紧急单总数`,
    COUNT(DISTINCT CASE WHEN urgent_flag = 0 AND is_normal_ontime = 1 THEN order_no END) AS `非紧急单准时上门数`,
    CONCAT(
        ROUND(
            CASE 
                WHEN COUNT(DISTINCT CASE WHEN urgent_flag = 0 THEN order_no END) = 0 THEN 0
                ELSE COUNT(DISTINCT CASE WHEN urgent_flag = 0 AND is_normal_ontime = 1 THEN order_no END) * 100.0 
                     / COUNT(DISTINCT CASE WHEN urgent_flag = 0 THEN order_no END)
            END, 
        2), '%'
    ) AS `非紧急单准时上门率`,
    ...
    -- 综合统计
    COUNT(DISTINCT CASE 
        WHEN (urgent_flag IN (1, 2) AND is_urgent_ontime = 1)  -- 紧急单（含紧急转非）
             OR (urgent_flag = 0 AND is_normal_ontime = 1)  -- 非紧急单
        THEN order_no 
    END) AS `准时上门总数`,
    CONCAT(
        ROUND(
...
GROUP BY city_name, bizcircle_name, order_month, urgent_flag_name
ORDER BY city_name, bizcircle_name, order_month, urgent_flag_name