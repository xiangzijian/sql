...
t1.`维修家电`,
t1.`维修综合`,
case when t2.`资质类型` = '燃气具安装维修工' then 1 else 0 end as `燃气有资质`,
case when t2.`资质类型` = '高空作业证' then 1 else 0 end as `家电有资质`,
case when t2.`资质类型` = '低压电工证' then 1 else 0 end as `综合有资质`,
t2.`资质类型` as `资质类型明细`
...
join 
  (select service_order_professional_ucid,
   ...
   and service_code = '10003'
        ...
        AND to_date(service_order_complete_time) >= date_sub(current_date, 30)
  group by 1,2,3) b
