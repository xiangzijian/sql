--模板查询：1月检修返修-商圈维度
--模板查询：新口径-检修返修（增加商圈维度，统计2025年6月-2026年1月）
...
WITH numbers AS (
    ...
    FROM (
        SELECT n, city_name
        FROM (SELECT stack(8, 6,7,8,9,10,11,12,13) AS n) t1  -- 6,7,8,9,10,11,12对应2025年6-12月, 13对应2026年1月
...
-- 第二步：核心出房房源+最后一次检修信息（通过订单表关联获取商圈）
t1 AS (
    SELECT 
        a.`城市名称`,
        COALESCE(ob.bizcircle_name, '') AS `商圈名称`,  -- 通过订单表关联获取商圈
        a.`出房起租月`,
        a.`出房起租日`,
        a.`资管公司名称`,
        a.`大区`,
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE pt = '20260129000000'
              AND task_type <> 12
              AND city_name in ('苏州市','成都市','南京市','上海市','北京市','天津市','宁波市','广州市','杭州市','武汉市','济南市','深圳市','西安市','上海市','北京市','天津市','宁波市','广州市','杭州市','武汉市','济南市','深圳市','西安市')
...
order_function AS (  -- 修正表名拼写错误（原rder_function）
  ...
  WHERE pt = '20260129000000' -- 统一pt分区
    AND function_name != '' 
...
base_complete_orders AS (
    ...
    FROM (
        ...
        WHERE pt = '20260129000000'
            ...
            AND TO_DATE(service_order_complete_time) BETWEEN '2025-06-01' AND '2026-01-31'  -- 修改为2025年6月-2026年1月
