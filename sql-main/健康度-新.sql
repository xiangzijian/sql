...
t2_agg AS (
    SELECT 
        ...
        count(distinct case when `call_time_1h` ='是' and performance_mode != '紧急单' and cancel_order='否' then order_no end) as `普通致电分子`,
        count(distinct case when performance_mode != '紧急单' and cancel_order='否'   and order_category='其他' then order_no end) as `普通致电分母`,
        count(distinct case when `call_time_30m`= '是' and performance_mode = '紧急单' and cancel_order='否' then order_no end) as `紧急致电分子`,
    ...
    WHERE pt='20260201000000' AND order_category='其他'
...
t3_agg AS (
    SELECT 
        ...
        city_name, 
        service_order_supplier_name,
        count(distinct case when performance_mode = 1 and `cancel_reason` not in ('夜间取消单', '白天致电前取消')  and `12time_2h` ='是' then order_no end) as `紧急上门分子`,
        count(distinct case when performance_mode = 1 and `cancel_reason` not in ('夜间取消单', '白天致电前取消') then order_no end) as `紧急上门分母`,
...
left JOIN t7_agg t7 
    ON t1.month_string = t7.exam_month 
    AND t1.city_name = t7.city_name 
    AND t2.service_order_supplier_name = t7.service_order_supplier_name
