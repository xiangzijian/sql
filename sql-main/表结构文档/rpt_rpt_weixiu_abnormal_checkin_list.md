# 表结构：rpt.rpt_weixiu_abnormal_checkin_list

## 表基本信息

- **数据库名**: `rpt`
- **表名**: `rpt_weixiu_abnormal_checkin_list`
- **表说明**: 维修异常签到列表（汇总表）
- **更新日期**: [待更新]
...
| 7 | 异常签到单量 | INT | YES | NULL | 异常签到单量 |
| 8 | 异常签到房源量 | INT | YES | NULL | 异常签到房源量 |
| 9 | 异常签到房源列表 | STRING | YES | NULL | 异常签到房源列表（逗号分隔） |
| 10 | 异常签到服务单列表 | STRING | YES | NULL | 异常签到服务单列表（逗号分隔） |
| 11 | 异常签到服务单时间 | STRING | YES | NULL | 异常签到服务单时间列表（逗号分隔） |
| 12 | 营销大区/大部 | STRING | YES | NULL | 营销大区/大部 |
| 13 | pt | STRING | NO | NULL | 分区字段（日期分区） |
...
- 此表是异常签到数据的汇总表
- 包含两种异常类型：短时间多次签到、异常距离签到
- 对于短时间多次签到，会记录窗口内的详细信息（房源列表、服务单列表等）
