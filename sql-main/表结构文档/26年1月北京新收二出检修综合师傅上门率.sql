...
repair_orders AS (
    SELECT DISTINCT
        ...
        a.label_group,
        a.lease_status,
        a.house_resource_id  -- 托管房源编码，用于关联房源信息
    ...
    WHERE a.pt = '20260127000000'
        AND a.city_name = '北京市'
        -- 检修单判断：label_group包含1或25
        AND (a.label_group LIKE '%1%' OR a.label_group LIKE '%25%' OR a.lease_status IN ('-1', '1'))
...
-- 通过托管房源编码关联房源信息（判断首出/二出）
LEFT JOIN house_info h
    ON CAST(r.house_resource_id AS BIGINT) = h.trusteeship_housedel_code
