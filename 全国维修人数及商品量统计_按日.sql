-- 统计24年12月24日-26年1月31日每天全国维修人数及商品量
-- 分维修类型：总/综合/家电，分租期维修/检修
-- 从order_commodity_list解析shelf_code，关联运力表获取commodity_code
SELECT 
    to_date(order_create_time) AS `日期`,
    
    -- ========== 维修人数统计 ==========
    COUNT(DISTINCT service_order_professional_ucid) AS `总维修人数`,
    COUNT(DISTINCT CASE WHEN `技能类型` IN ('维修综合', '维修综合家电') THEN service_order_professional_ucid END) AS `综合维修人数`,
    COUNT(DISTINCT CASE WHEN `技能类型` IN ('维修家电', '维修综合家电') THEN service_order_professional_ucid END) AS `家电维修人数`,
    
    -- ========== 租期维修商品量统计 ==========
    COUNT(DISTINCT CASE 
        WHEN label_group NOT IN ('1', '8', '25') 
        AND lease_status IN (2, 3)
        THEN CONCAT(order_no, '-', commodity_code)
    END) AS `租期维修总商品量`,
    ...
    -- ========== 紧急商品量统计 ==========
    COUNT(DISTINCT CASE 
        WHEN performance_mode IN (1, 2)  -- 1=紧急单, 2=加急单
        THEN CONCAT(order_no, '-', commodity_code)
    END) AS `紧急商品量`
...
FROM (
    SELECT 
        ...
        a.shelf_code,
        b.commodity_code,
        -- 判断维修类型（基于commodity_code，用于商品量统计）
        CASE 
    ...
    FROM (
        ...
        AND to_date(order_create_time) BETWEEN '2024-12-24' AND '2026-01-31'
